...
public class DPS310 {
    ...
    public void run() throws IOException, I2CFactory.UnsupportedBusNumberException {
        ...
        I2CDevice device = i2c.getDevice(DPS310_ADDRESS);
        int response = device.read();
        log.warn("... reading ID register from DPS310 " + response + "  " + Integer.toString(response, 2));
//        console.println("TSL2561 ID = " + String.format("0x%02x", response) + " (should be 0x50)");
        ...
        try {
            ...
            while (check < 192) {  //192
                Thread.sleep(500);
                device.write(MEAS_CFG);
                check = device.read();
                log.warn("check = " + Integer.toString(check, 2));
            }

            device.write((byte) 0x28);
            int sensorType = device.read();
            boolean internalSensor = Integer.toString(sensorType, 2).charAt(0) == '0';
            log.warn("internalSensor = " + internalSensor);


            tempOversampling = oversampling.get(16);
            prsOversampling = oversampling.get(16);
            String s = "";
            device.write((byte) MEAS_CFG);
            int tt = device.read();
            log.warn("check = " + ((tt & MEAS_CFG_TMP_RDY) > 0) + "   " + Integer.toString(tt, 2));
//fill coeff
            int[] coef = new int[18];
            device.write((byte) 0x10);
            for (int i = 0; i < coef.length; i++) {
                coef[i] = device.read();
                log.warn("coef[i] = " + coef[i] + "   " + Integer.toString(coef[i], 2));
            }
            tempCoef0Half = complement((coef[0] << 4) | (coef[1] >> 4), 12) / 2.0;
            log.warn("m_c0Half = " + tempCoef0Half);
            tempCoef1 = complement((coef[1] & 0x0F) << 8 | coef[2], 12);
            log.warn("m_c1  = " + tempCoef1);
...
//
            int zz = 0;
            while (zz++ < 5) {
                device.write(MEAS_CFG, (byte) 0x02);
                int[] t = new int[3];
                ...
                log.warn("temRaw = " + temRaw);
                tempRawSc = temRaw / tempOversampling.getKey();
                double tempValue = tempCoef0Half + tempCoef1 * tempRawSc;
                log.warn("temperature  = " + tempValue);

                log.warn(" =----------------------------------------- ");
                device.write(MEAS_CFG, (byte) 0x01);
                Thread.sleep(1000);
                device.write((byte) 0x00);
                for (int i = 0; i < t.length; i++) {
                    t[i] = device.read();
                    log.warn(" " + t[i] + " " + Integer.toString(t[i], 2));
                }
...
//                        io.write(TMP_CFG);
//                       log.warn("temp check = " + Integer.toString(io.read(), 2));
//                        io.write(CFG_REG, 0x0C);
//                        io.write(CFG_REG);
//                       log.warn(" check = " + Integer.toString(io.read(), 2));
//                    }
//                    if ("0".equalsIgnoreCase(s)) {
//                        io.write(INT_STS);
//                       log.warn("int = " + Integer.toString(io.read(), 2));
//                    }
//                    if ("1".equalsIgnoreCase(s)) {
//                        io.write(MEAS_CFG);
//                        int t = io.read();
//                       log.warn("check = " + ((t & MEAS_CFG_TMP_RDY) > 0) + "   " + Integer.toString(t, 2));
//                    }
//                    if ("2".equalsIgnoreCase(s)) {
//                        io.write(FIFO_STS);
//                       log.warn("fifo = " + Integer.toString(io.read(), 2));
//                    }
